...
%   Detailed explanation goes here
   ...
   methods
       function appData = analyze(obj, appData) % do the analysis
           ...
           firstGuess = [fitObj.OD*0.85, 0.2, fitObj.C,  3.5, 0, 0.1, fitObj.sigmaY, fitObj.y0-y0];
           lower = [0 0 -0.1 0 -Inf 0 0 0 ];
           upper = [10 5 0.1 20 Inf 1 length(yData) length(yData) ];
%            firstGuess = [10, 0, 0.1];
%            lower = [ 0 -Inf 0  ];
%            upper = [ 20 Inf 1 ];
           fo = fitoptions('method','NonlinearLeastSquares','Robust','On', ...
               'DiffMinChange',1e-010,'DiffMaxChange',1e-005,'MaxFunEvals',10000, ...
               'MaxIter',10000,'TolFun',1e-010,'TolX',1e-010, 'Startpoint', firstGuess, ...
               'Lower', lower, 'Upper', upper);
            ft = fittype('a*max(1-(x-x0)^2/w^2, 0)^(3/2)*(1+v*sin(2*pi/lambda*(x-x0)+phi)) / (1+a0*max(1-(x-x0)^2/w^2, 0)^(3/2)*(1+v*sin(2*pi/lambda*(x-x0)+phi)))+c',...
                'dependent',{'y'},'independent',{'x'}, 'coefficients',{'a', 'a0', 'c', 'lambda', 'phi', 'v', 'w', 'x0'});
%             ft = fittype('a*exp(-(x-x0)^2/2/w^2)*(1+v*sin(2*pi/lambda*(x-x0)+phi)) / (1+a0*exp(-(x-x0)^2/2/w^2)*(1+v*sin(2*pi/lambda*(x-x0)+phi)))+c',...
%                 'dependent',{'y'},'independent',{'x'}, 'coefficients',{'a', 'a0', 'c', 'lambda', 'phi', 'v', 'w', 'x0'});
            obj.a = fitObj.OD;
            obj.x0 = fitObj.y0-y0;
            obj.w = fitObj.sigmaY;            
...
%            close(h);
       ...
       function [xFit yFit] = getXYFitVectors(obj, x, y)
           ...
           y = y-obj.yStart+1;
%            y = [y(1):0.01:y(end)];
           yFit =  obj.res.a*max(1-(y-obj.res.x0).^2/obj.res.w^2, 0).^(3/2) .*  ...
               (1+obj.res.v*sin(2*pi*(y-obj.res.x0)/obj.res.lambda+obj.res.phi)) ./ ...
               (1+obj.res.a0*max(1-(y-obj.res.x0).^2/obj.res.w^2, 0).^(3/2) .*  ...
               (1+obj.res.v*sin(2*pi*(y-obj.res.x0)/obj.res.lambda+obj.res.phi)))...
               +obj.res.c;
