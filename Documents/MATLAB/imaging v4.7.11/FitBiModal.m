classdef FitBiModal < FitTypes
%FIT2THOMASFERMI Summary of this class goes here
...
%            normalizedROI = obj.OD * max(1 - ( (X-obj.x0)./ (2*obj.TFhwX) ).^2 - ( (Y-obj.y0)./(2*obj.TFhwY) ).^2 ,0).^(3/2);
           normalizedROI = obj.ampTF * max(1 - ( (X-obj.x0)./ (1*obj.TFhwX) ).^2 - ( (Y-obj.y0)./(1*obj.TFhwY) ).^2 ,0).^(3/2) + ...
       ...
       function [xFit yFit] = getXYFitVectors(obj, x, y)
           xFit = obj.ampTF * max( 1 - ( (x-obj.x0)./(1*obj.TFhwX) ).^2, 0 ).^(3/2) + ...
               obj.ampG*exp( -0.5 * ( (x-obj.x0).^2 / obj.sigmaX^2 ) ) + obj.C;
           yFit = obj.ampTF * max( 1 - ( (y-obj.y0)./(1*obj.TFhwY) ).^2, 0 ).^(3/2) + ...
