...
classdef GeneralLoop_1 < Measurement
...
%         pmMAD = {};
     ...
     methods ( Static = true )
        function o = create(appData)
            o = GeneralLoop_1(appData);
        end
     end
     
    methods        
        function obj = GeneralLoop_1(appData)
            % first line - MUST
...
%             obj.saveParamVals = appData.consts.saveParamValDefault;
...
%                 end
            ...
            obj.etSaveParamVals = uicontrol(obj.win, ...
                'Style', 'edit', ...
                'String', '', ... % num2str(obj.noIterations), ...
                'Units', 'pixels', ...
                'Enable', enableStr, ...
...
%                     obj.pmMAD{i}{j} = uicontrol(obj.pALoops(i), ...
%                         'Style', 'popupmenu', ...
%                         'String', {'Analog Channel' 'Digital Channel'}, ...
%                         'Value', 1, ...
%                         'Units', 'pixels', ...
%                         'Enable', enableStr, ...
...
%                     set(obj.pmMAD{i}{j}, 'Callback', {@obj.pmMAD_Callback});  
...
%             o.saveParamVals = get(obj.etSaveParamVals, 'Value');
%             try
...
%              obj.saveParam = get(object, 'Value');
            if ( obj.saveParam == length(obj.saveParamsStr) )
                param = inputdlg('Enter param name:', 'Other param input');
                set(obj.pmSaveParam, 'String', {obj.saveParamsStr{1:end-1} ['O.P. - ' param{1}]});
%                 obj.saveOtherParamStr = param;
...
%                     obj.pmMEventName_Callback( obj.pmMEventName{i}{j},  []);
...
%             obj.MEventName{i}{j} = get(object, 'Value');
...
%         function pmMAD_Callback(obj,  object, eventdata)
%             obj.pmMAD = get(object, 'Value');
%         end
        function pmMChannelName_Callback(obj,  object, eventdata)
            [i j] = getPCIndex(obj, object, obj.pmMChannelName);
%             obj.MChannelName{i}{j} = get(object, 'Value');
